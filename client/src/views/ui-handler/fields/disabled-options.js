    ...
    return Dep.extend({
        ...
        afterRender() {
            ...
            if (field && this.model.get('type') === 'ui_disable_options') {
                ...
                if (defs && ['enum', 'multiEnum', 'extensibleEnum', 'extensibleMultiEnum'].includes(defs['type'])) {
                    const type = ['enum', 'multiEnum'].includes(defs['type']) ? 'multiEnum' : 'extensibleMultiEnum'
