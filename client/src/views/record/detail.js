...
Espo.define('views/record/detail', ['views/record/base', 'view-record-helper'], function (Dep, ViewRecordHelper) {
    ...
    return Dep.extend({
        ...
        delete: function () {
            ...
            this.confirm({
                message: (this.translate(parts.pop(), parts.pop(), parts.pop())).replace('{{name}}', this.getModelTitle(this.model)),
        ...
        cleanDuplicatedButtons() {
            ...
            buttonTypes.forEach((type) => {
                ...
                this[type] =  this[type].filter(el => {
                    if(!existing.includes(el.action || el.name)) {
                        existing.push(el.action || el.name);
                        return true;
                    }
                    return false;
                });
