    ...
    return Dep.extend({
            ...
            this.getCollectionFactory().create(this.scope, function (collection) {
                ...
                this.once('after:render', function () {
                    this.createView('list', viewName, {
                        ...
                        disableSorting: !!this.defs.disableSorting,
                        pagination: 'bottom'
                    }, (view) => {
        ...
        setupTotal() {
            this.listenTo(this.collection, 'update update-total', () => {
                ...
                if (total === null) {
                    ...
                    $buttonHtml.attr('title', `${this.translate('Total', 'labels', 'Global')}: ${total}`)
                    $buttonHtml.find('span').html(total);
                    $buttonHtml.removeClass('hidden')
                }
            })
        },
        ...
        actionUnlinkAllRelated: function (data) {
            this.confirm(this.translate('unlinkAllConfirmation', 'messages'), function () {
                ...
                const total = this.collection.total;
                const threshold = this.getConfig().get('maxMassUnlinkCount') || 20;
                const onDone = () => {
                    this.collection.fetch();
                    this.model.trigger('after:unrelate', this.link, this.defs);
                };
