...
Espo.define('views/selection/detail', ['views/detail', 'model', 'views/record/list'], function (Dep, Model, List) {
    ...
    return Dep.extend({
        ...
        reloadModels(callback) {
            this.loadSelectionItemModels(this.getItemsUrl(this.model.id)).then(models => {
                if (models.length > 0) {
        ...
        actionMerge() {
            ...
            this.getMainRecord()?.applyMerge((result) => {
                this.selectionViewMode = 'compare';
                this.updateUrl(this.selectionViewMode, true);
            });
        },
