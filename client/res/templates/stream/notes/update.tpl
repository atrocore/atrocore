    ...
    <div class="stream-head-container">
        ...
        <div class="stream-head-text-container">
              ...
              {{#if showDiff }}
                     {{#each fieldsArr}}
                <span class="text-muted message">{{{../message}}} <code>{{label}}</code></span> <a href="javascript:" data-action="expandDetails"><span class="fas fa-angle-down"></span></a>
                    {{/each}}
              {{/if}}
        </div>
    </div>
    ...
    <div class="hidden details stream-details-container">
        ...
        <div class="cell col-md-12 col-lg-6 form-group">
            <label class="control-label">{{label}} {{translate 'was' category='streamMessages' scope='Global'}}</label>
