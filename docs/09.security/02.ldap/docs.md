...
After the module is installed, the LDAP panel appears in [`Administration → Authentication`](../../02.atrocore/03.administration/14.access-management/05.authentication/docs.md). Activate LDAP Authentication and configure the connection to your LDAP server.
...
When a user logs in for the first time, a new user record is created in AtroCore. A default role and team are assigned automatically. Both can be changed by the administrator. See [Access management](../../02.atrocore/03.administration/14.access-management/) for details.
