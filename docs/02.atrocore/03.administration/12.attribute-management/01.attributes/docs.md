...
| Channel                  | Allows you to set channel-specific attribute values. If a channel is added to an attribute, its name will be automatically added to the attribute name on the record page (for example, Image / Amazon). [Channels](../../../../06.pim/06.channels/docs.md) are only available with the PIM module installed.
...
The attribute code is also used to reference attribute values in [script-type](../../../../11.developer-guide/80.twig-tutorial/) fields and serves as the key name for attributes in [API](../../../../11.developer-guide/10.rest-api/) responses. If code is not specified, the attribute's unique ID is used as a fallback.
...
If you have a list of the records attributes with predefined values, it is convenient to set them using import. Learn more about adding attributes via import [here](../../../../03.data-exchange/01.import-feeds/docs.md#attributes).
...
The channel functionality is provided by the [PIM](../../../../06.pim/06.channels/docs.md) module and is primarily used in products for exporting data to various destinations.
...
> For bulk replacement, [export](../../../../03.data-exchange/02.export-feeds/docs.md) the records, update the attribute type and name to match the newly created attribute, then reimport them via [import](../../../../03.data-exchange/01.import-feeds/docs.md).
