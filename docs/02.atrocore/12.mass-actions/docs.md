...
taxonomy:
    category: docs
---
Mass actions are applied to one or more selected entity records from [list view](../../02.atrocore/04.understanding-ui/docs.md#list-view), i.e. records with a tick in the checkbox.
Use [search and filtering](../../02.atrocore/11.search-and-filtering/) to find what you need and check all results you need.
...
Additional options may be added by additional modules, such as **Translate** option added by [Translations](../../05.collaboration/04.translations) module. You can also [create](#custom-mass-actions) your own mass actions.
...
Users can export via the `Mass Actions` option. To do this, first select the records you want to update. Then select Actions/Export to proceed to the export pop-up. After making your selection, press the `Export` button. The pop-up will close and a new export execution will be created. You will see the export result in [Notifications](../../02.atrocore/03.administration/10.notifications/).
...
This will use [Export Feeds](../../03.data-exchange/02.export-feeds/) logic but only for the products you selected.
...
> Delete Attribute function is only visible on entities that have enabled attributes. For more information on attribute management, please refer to the [Attribute Management](../../02.atrocore/03.administration/12.attribute-management/) documentation..
