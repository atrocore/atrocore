...
Workflows are created in the same way as other entity [Records](../../01.atrocore/08.record-management/docs.md#creating-records). The system allows you to configure an unlimited number of workflows, and multiple workflows can be attached to a single entity.
...
Script is more complicated use case but as a result more versatile. It uses [Twig syntax](../../10.developer-guide/80.twig-tutorial/docs.md) for formulas. To use it you have to set a formula that results in a default Boolean variable "proceed" ({% set proceed = ... %}).
...
### Workflow Actions
 ...
 Workflow [Actions](../../01.atrocore/03.administration/06.actions/docs.md#automatic-execution) are the a separate entity. They are the essential for workflows to work. Workflow actions define what happens when the workflow is triggered and conditions are met. Each workflow can execute one defined action, which can be either a single action or an [Action Set](../../01.atrocore/03.administration/06.actions/docs.md#action-set) containing multiple actions.
...
For more details, refer to the documentation on [Action Types](../../01.atrocore/03.administration/06.actions/docs.md#action-types).
