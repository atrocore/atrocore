...
If a template for creating PDF for a single item (e.g., Product) is available, a "Generate PDF" option will appear as a [single record action](../../01.atrocore/08.record-management/docs.md#single-record-actions) in both the detail view and list view of the respective entity.

- From [Detail View](../../01.atrocore/04.understanding-ui/docs.md#detail-view). The PDF generation option appears in the actions menu on the detail page:

![pdf-generator-function](_assets/pdf-generator-function.png){.medium}

- From [List View](../../01.atrocore/04.understanding-ui/docs.md#list-view). The same option is also available from the list view through the record actions menu:
...
PDF templates markup fields use the [TWIG](../../10.developer-guide/80.twig-tutorial/) template engine.
...
it will be in [Job manager](../../01.atrocore/05.toolbar/03.job-manager/)).
...
You can configure the [filter](../../01.atrocore/11.search-and-filtering/) to specify for which entity records the PDF file generation will be available.

![PDF Feed filter](_assets/pdf-feed-filter.png){.medium}

Since you can configure automatic PDF generation via [Scheduled Jobs](../../01.atrocore/03.administration/05.system-jobs/01.scheduled-jobs/docs.md#automatic-pdf-generation), it is possible to set separate filters for manual and automatic generation.
...
In addition to the standard action for calling up the modal window for generating PDF files, it is also possible to define your own [Action](../../01.atrocore/03.administration/06.actions/docs.md#generate-pdf).
...
If the PDF Feed is executed for multiple records (for example, via an [action](../../01.atrocore/03.administration/06.actions/docs.md#generate-pdf) or a [scheduled job](../../01.atrocore/03.administration/05.system-jobs/01.scheduled-jobs/docs.md#automatic-pdf-generation)), it can be created using multiple jobs in parallel. By default, one job can generate PDFs for a hundred records. This can be changed by editing the `entitiesPdfPerJob` parameter in the configuration file.
