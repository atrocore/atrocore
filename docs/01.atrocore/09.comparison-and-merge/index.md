...
1. Via the [Layout Manager](../03.administration/13.user-interface/02.layouts/index.md#configuration-options)
...
For [many-to-many](../03.administration/11.entity-management/07.fields-and-relations/index.md#many-to-many-relationships) relationships, select the checkbox next to all records that should be linked to the merged record.
...
You will see the message “Merged” on the screen and will be redirected to the newly created record, which means that the resulting record has been saved and the rest of the records have been [deleted](../08.record-management/index.md#deleting-records). As with any deleted records, they will be stored in the database for a certain period of time and you can restore them if necessary.
...
Selection is a standard entity that can be chosen within the scope of entities in role settings. You can configure the same [access permissions](../03.administration/14.access-management/index.md) for Selections as for any other entity (create, read, edit, delete).
