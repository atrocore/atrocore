...
Use the [global search](../05.toolbar/index.md#global-search) if you wish to search globally across all the entities in the system.
...
Search allows quick access to records by entering text that matches field values. These sections are automatically available on each entity [list](../04.understanding-ui/index.md#list-view) view page:
...
The search function looks through the record [identifiers](../03.administration/11.entity-management/02.data-types/index.md#identifiers), text fields ([string](../03.administration/11.entity-management/02.data-types/index.md#string), [text](../03.administration/11.entity-management/02.data-types/index.md#text), [email](../03.administration/11.entity-management/02.data-types/index.md#email), [URL](../03.administration/11.entity-management/02.data-types/index.md#url), [HTML](../03.administration/11.entity-management/02.data-types/index.md#html)), and numeric fields ([integer](../03.administration/11.entity-management/02.data-types/index.md#integer), [float](../03.administration/11.entity-management/02.data-types/index.md#float)). Search is case-insensitive.
...
This behavior can be modified for text fields through the `Use 'contains' operator when filtering varchar fields` setting in [System Settings](../03.administration/01.system-settings/index.md). When enabled, String fields use "contains" matching for queries 4 characters or longer, similar to text fields.
...
- **Active** – active records only (entities with an `isActive` field, configured via [Entity Management](../03.administration/11.entity-management/index.md#configuration-fields)).
- **My / Owned by me / Assigned to me** – records associated with, owned by, or assigned to the current user (entities with [Owner or Assigned User](../03.administration/11.entity-management/index.md#access-management-panel) enabled).
- **Multiple Classifications** – records with more than one [classification](../03.administration/12.attribute-management/04.classifications/index.md) assigned.
- **Without main image** – records with no main image set ([Products](../../05.pim/03.products/index.md) only).
...
The **Advanced Filter** is available in the list view for all Basic, Archive and Hierarchical [entities](../03.administration/11.entity-management/index.md).
...
Depending on the [field/attribute type](../03.administration/11.entity-management/02.data-types/index.md), the following filtering criteria can be applied:
...
| **Link, Multiple Link, Measure, Array, List, Multi-value List, Currency List, Language Code, Language Codes, Route(s)** | In / Not In | Value list / multiselect |
| | Is Null / Is Not Null | – |
...
To enable correct filtering across multiple measurement units, conversion factors must be defined in the Measures configuration. Navigate to [Measures](../03.administration/09.measure-units/index.md) and configure the `Multiplier` parameter for each unit. The multiplier defines the conversion ratio relative to the base unit of the measure.
...
For all entities there is an additional panel where records can be searched and filtered by Link or [Link Multiple](../03.administration/11.entity-management/07.fields-and-relations/index.md) fields. For example, for the Product entity, these can be Categories, Brands, Classifications, Catalogs, etc.
...
In addition to relationship fields, there is a [Bookmarks](../05.toolbar/01.bookmarks/index.md) link at the top of the left sidebar.
