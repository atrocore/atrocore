...
| -------------- | ---------------------------------------------------------------------------------------------------------------------------------------- |
| Name           | Field name in the main language. Can be localized — see [Labels](../../13.user-interface/03.labels/index.md#alternative-editing-methods). |
...
-   **Required**: Field must be filled in (Not available for [Multiple Link](../02.data-types/index.md#multiple-link) fields).
...
- **Tooltip**: Enables contextual help for the field, supporting text and links. Can be localized — see [Labels](../../13.user-interface/03.labels/index.md#alternative-editing-methods).
...
- **Is Code**: Marks the field as the entity's code field. When enabled, records can be looked up by this field's value instead of their internal ID — for example, in URLs (`Product/NB.X003` instead of `Product/019c7a32-f6f4-72dc-82b1-13c47b8bf61c`). Only one field per entity can be marked as code. Available for:
...
For [Link](../02.data-types/index.md#link) and [Multiple Link](../02.data-types/index.md#multiple-link) data types, a **Filter Result** panel is available. This panel appears only for these two types and shows which options will be available to the user based on the configured filter.
...
Conditional properties are configured through the [Basic conditions type](#basic-conditions-type). This functionality enables administrators to define specific field properties that are applied automatically to [list](../../../04.understanding-ui/index.md#list-view) and [detail](../../../04.understanding-ui/index.md#detail-view) views when predefined conditions are met.
...
> **Auto-increment fields**: Conditions based on an [Auto-increment](../02.data-types/index.md#auto-increment) field are only evaluated on already-saved records. During initial record creation, no value exists yet, so the condition will not be triggered.
