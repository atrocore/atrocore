...
Classifications are most commonly used with [Products](../../../../05.pim/03.products/) but can be enabled for any entity via the [Entity Manager](../../11.entity-management/docs.md). One attribute can be used in multiple Classifications, and one Classification can have many attributes. Each record can be assigned to multiple Classifications.
...
- **Entity** — The entity this Classification applies to. For [Listings](../../../../05.pim/14.listing/docs.md) a **Channel** field will also appear. Cannot be changed after creation.
...
- **Select Attribute(s)** — opens a selection widget to choose one or more existing [attributes](../01.attributes/docs.md)
- **Select Attribute Group** — assigns all attributes belonging to an [Attribute Group](../02.attribute-groups/docs.md) at once

Use the filter sidebar within each widget to narrow down the results.

To create and immediately link a new attribute, click the `+` icon in the panel header. The attribute creation widget will open — see [Attribute Fields](../01.attributes/docs.md#attribute-fields) for field descriptions.
...
- **Edit** — opens an editor to configure classification-level settings, including whether the attribute is required, its default value, and any conditional properties (see [Attribute Fields](../01.attributes/docs.md#attribute-fields))
