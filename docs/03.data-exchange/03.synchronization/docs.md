...
> Menu item "Synchronizations" should be [added](../../02.atrocore/03.administration/13.user-interface/01.navigation/docs.md#adding-and-removing-items) by your Administrator. 
...
! You can create a synchonization [Action](../../02.atrocore/03.administration/06.actions/docs.md#synchronization) to operate with your synchronization in [Scheduled jobs](../../02.atrocore/03.administration/05.system-jobs/01.scheduled-jobs/docs.md) or [Workflows](../../05.collaboration/01.workflows/docs.md).
...
By default, feeds execute sequentially - each feed starts only after the previous one completes with a [`Success`](../../02.atrocore/03.administration/05.system-jobs/docs.md#job-statuses) status. If any feed fails, the remaining feeds in the sequence are cancelled and will not execute.

> You can override this behavior by enabling the `Force executing` checkbox in the [Detail view](../../02.atrocore/04.understanding-ui/docs.md#detail-view) of the Synchronization record. When enabled, the synchronization will continue executing all feeds in sequence even if previous feeds encounter errors, ensuring that all feeds are attempted regardless of individual feed failures.
...
You can monitor export and import errors by using "Data sync errors for Import" and "Data sync errors for Export" widgets by adding them to [Dashboard](../../02.atrocore/07.dashboards/docs.md) via `Add Dashlets`.
...
Combine [Remote File](../06.import-feeds-remote-file/docs.md) import feeds to process files from different FTP/SFTP locations or network paths in a coordinated sequence. Configure synchronizations to run automatically via [Scheduled Jobs](../../02.atrocore/03.administration/05.system-jobs/01.scheduled-jobs/docs.md) for regular data updates without manual intervention. 
