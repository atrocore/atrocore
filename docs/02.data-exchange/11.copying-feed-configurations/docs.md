...
Go to Administration → Tools and select [API Request](../../01.atrocore/03.administration/16.tools/01.api-request/). Select the type Upsert and copy the script from the clipboard into the request field. This field will only accept JSON codes. Then press Execute. You will see a response in the response field.
