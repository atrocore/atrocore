...
!! Users must have the following permissions configured via [Roles](../../01.atrocore/03.administration/14.access-management/03.roles/index.md) (Scopes panel): `Import Feeds`, `Import Execution` and `Files`, as well as at least Read permission for `Folder`, and `Storage`. Without these, feed import execution will be denied. In [Access Control List](../../01.atrocore/03.administration/14.access-management/index.md#acl-strict-mode) strict mode, these permissions must be granted explicitly — they are not given by default.
...
- **Folder** – the [folder](../../01.atrocore/03.administration/15.file-management/index.md#folder-management) where files created by this import feed will be stored. This field is required.  
...
- **Execute As** – user context for import execution:
    ...
    - *Same user* – runs with current user permissions.  When selected, the corresponding user appears as a link following System in the **Created** and **Modified** fields in the [Summary](../../01.atrocore/04.understanding-ui/index.md#insights-tab) panel of the Side View for changed records.
...
Use [single record actions](../../01.atrocore/04.understanding-ui/index.md#single-record-actions) to edit or delete mapping rules.
...
For [Boolean](../../01.atrocore/03.administration/11.entity-management/02.data-types/index.md#boolean) fields/attributes:
...
For [Integer Range](../../01.atrocore/03.administration/11.entity-management/02.data-types/index.md#integer-range) and [Float Range](../../01.atrocore/03.administration/11.entity-management/02.data-types/index.md#float-range) fields and attributes, values must be imported from two separate columns in your source file:
...
[Numeric](../../01.atrocore/03.administration/11.entity-management/02.data-types/index.md#numeric-data-types)/[string](../../01.atrocore/03.administration/11.entity-management/02.data-types/index.md#string) fields with measure units accept values like "9 cm", "110,50 EUR", "100.000 USD".
...
View last execution time and status on import feed [details](../../01.atrocore/04.understanding-ui/index.md#detail-view) and [list](../../01.atrocore/04.understanding-ui/index.md#list-view) views:
...
Files generated during import are linked to both their Import Feed and Import Execution (field name Import Job). You can view or filter files by these fields in the [File](../../01.atrocore/13.file-operations/index.md) entity to trace their origin.
...
Standard [record management actions](../../01.atrocore/08.record-management/index.md#single-record-actions) are available, including **duplicate** and **remove**.
...
**Copy Configurations** exports configuration as JSON. See [Copying Feed Configurations](../11.copying-feed-configurations/index.md) for details.

### Creating Import Feed from Export Feed

With the [Export Feed](../02.export-feeds/index.md) module installed, create import feeds from export feeds using `Duplicate as import`:
...
!! Mapping rules for protected fields are excluded, as [protected](../../01.atrocore/03.administration/11.entity-management/03.fields-and-attributes/index.md#protected) fields cannot be imported.