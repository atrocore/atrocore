...
Module [Export: HTTP Request](https://store.atrocore.com/en/export-http-request/201400.1) extends [Export Feeds](../02.export-feeds/index.md) to automate data exports by sending exported data directly to external systems via HTTP endpoints—eliminating the need to manually transfer files after export.

Connect to any HTTP-based target—REST APIs, web services, or custom endpoints—and push data directly from AtroCore without intermediate file handling. Combined with [Scheduled Jobs](../../01.atrocore/03.administration/05.system-jobs/01.scheduled-jobs/index.md), you can fully automate recurring exports to keep external systems synchronized with your PIM data.
...
Export feed configuration works the same for all export feed types, except for the fields specific to each type. See [Export Feeds](../02.export-feeds/index.md) for general setup—this guide covers only the HTTP Request-specific fields.
...
The `Details` panel contains [fields](../02.export-feeds/index.md#details-panel) common to all export feed types.
...
- **Format** – the data format in which records are serialized before being sent in the request body. Available formats are **JSON** and **XML**. For both, a `Template` field is also available for defining the output structure using [Twig syntax](../../10.developer-guide/80.twig-tutorial/index.md).
...
- **Connection** – select an existing [Connection](../../01.atrocore/03.administration/04.connections/index.md) entity or create a new one. The Connection stores authentication credentials (such as API keys, tokens, or username/password) securely and separately from the feed configuration.
...
> Once a **Format** is selected, the **Feed Settings** panel is displayed, where you can configure the export options as described in [Feed Settings](../02.export-feeds/index.md#feed-settings).
...
All other aspects of export feed configuration and usage are the same as for file-based exports: field mapping in the [Configurator](../02.export-feeds/index.md#configurator), [running exports](../02.export-feeds/index.md#running-an-export-feed), [export executions](../02.export-feeds/index.md#export-executions).

For full details on export execution states, downloaded files, and error handling, see [Export Executions](../02.export-feeds/index.md#export-executions) in the Export Feeds documentation.