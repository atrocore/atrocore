...
Before creating use cases, the connection to an AI engine must be set up. It is set up in [connections](../../01.atrocore/03.administration/04.connections/docs.md#ai-services).
...
The AI Integration module enables users to send prompts directly to an AI engine. Users can use the values of any field or attribute of a record as text or files to send them as images to an AI engine. This is achieved using [Twig](../../10.developer-guide/80.twig-tutorial) guidelines and syntax. The responses provided will be used as data for the field and must be in text format only.
