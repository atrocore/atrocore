...
In Atro system Classifications are used in order to define a set of [attributes](../../02.atrocore/03.administration/12.attribute-management/01.attributes/) that are shared by entity records belonging to a certain family, and to describe the characteristics of these records. For each Classification you can define, which attributes are mandatory and which are optional, so the system can calculate the completeness of your content.
...
Classification records can be searched and filtered according to your needs. For details on the search and filtering options, refer to the [**Search and Filtering**](../../02.atrocore/11.search-and-filtering/) article in this user guide.

To view the details of a classification record, click on the name of the corresponding record in the list of classifications. The [detail view](../../02.atrocore/04.understanding-ui/docs.md#detail-view) page will open showing the Classification records and the records of the related entities. Alternatively, select the `View` option from the single record actions menu to open the [quick detail](../../02.atrocore/04.understanding-ui/docs.md#quick-detail-view-small-detail-view) pop-up.
...
To edit the Classification, click the `Edit` button on the [detail view](../../02.atrocore/04.understanding-ui/docs.md#detail-view) page of the currently open Classification record; the following editing window will open:
...
Besides, you can make changes in the Classification record via [in-line editing](../../02.atrocore/08.record-management/docs.md#in-line-editing) on its detail view page.

Alternatively, make changes to the desired Classification record in the [quick edit](../../02.atrocore/04.understanding-ui/docs.md#quick-edit-view) pop-up that appears when you select the `Edit` option from the single record actions menu on the Classifications list view page:
...
Relations to [attributes](../../02.atrocore/03.administration/12.attribute-management/01.attributes/) and [products](../03.products/) are available for all Classifications by default. The related entities records are displayed on the corresponding panels on the Classification [detail view](../../02.atrocore/04.understanding-ui/docs.md#detail-view) page. If any panel is missing, please, contact your administrator as to your access rights configuration.
...
Attributes that are linked to the Classification record are shown on the `Classification Attributes` panel within the Classification detail view page and include the following table columns:
 - Attribute
 - Required
 - Audit Completeness (available only with [data quality](../../02.atrocore/15.data-quality) module)
 - Default Value
...
AtroPIM supports linking to Classifications not only separate attributes, but also [attribute groups](../../02.atrocore/03.administration/12.attribute-management/02.attribute-groups/). For this, use the `Select Attribute Group` option from the actions menu, and in the "Attribute Groups" pop-up that appears, select the desired groups from the list of available attribute groups.
...
To view the attribute/attribute group record from the `Classification Attributes` panel, click its name in the attributes list. The [detail view](../../02.atrocore/04.understanding-ui/docs.md#detail-view) page of the given attribute/attribute group will open, where you can perform further actions according to your access rights.
...
Then the record page opens, where all entity records [filtered](../../02.atrocore/11.search-and-filtering/) by the given Classification are displayed.
...
To view the related entity record from the entity panel, click on its name in the records list. The [detail view](../../02.atrocore/04.understanding-ui/docs.md#detail-view) page of the given record will open, where you can perform further actions according to your access rights.
