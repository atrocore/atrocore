...
class FieldManagerController extends AbstractListener
{
    ...
    protected function isUniqueFieldWithoutDuplicates(string $scope, string $field)
    {
        ...
        if (isset($defs['type'])) {
            switch ($defs['type']) {
                case 'asset':
                    $sql = "SELECT COUNT(*) FROM $table JOIN attachment a ON a.id = $table.{$field}_id WHERE $table.{$field}_id IS NOT NULL AND $table.deleted = 0 GROUP BY a.md5 HAVING COUNT(a.md5) > 1";
                    break;
                case 'currency':
