...
#[Route(
    path: '/Layout/selectionRelations',
    methods: [
        'GET',
    ],
    summary: 'Get selection relations layout for an entity',
    description: 'Returns the selection relations layout for the given entity. This layout controls which relationship panels are available in the record-picker selection context.

**How to use:**
- `entityName` — the entity whose selection relations layout is requested.
...
- `layoutProfileId` — optional layout profile ID.',
    ...
    parameters: [
        ...
        [
            ...
            'in'          => 'query',
            'required'    => false,
            'description' => 'Custom layout name from `additionalLayouts`. Overrides the default `selectionRelations` view type.',
            'schema'      => [
                'type' => 'string',
            ],
        ],
    ...
    responses: [
        200 => [
            'description' => 'Selection relations layout for the requested entity',
            'content'     => [
                'application/json' => [
                    'schema' => [
                        'type'       => 'object',
                        ...
                        'properties' => [
                            'layout'            => [
                                'type'        => 'array',
                                'description' => 'Ordered list of relationship panel entries available in selection context.',
                                'items'       => [
...
class LayoutSelectionRelationsHandler extends AbstractHandler
{
    public function process(ServerRequestInterface $request, RequestHandlerInterface $handler): ResponseInterface
    {
        $qp       = $request->getQueryParams();
        $viewType = $qp['layoutName'] ?? 'selectionRelations';
        ...
        if (empty($data)) {
            throw new NotFound("Selection relations layout for '{$qp['entityName']}' is not found.");
        }
