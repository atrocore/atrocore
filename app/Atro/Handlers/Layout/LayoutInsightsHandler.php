...
#[Route(
    path: '/Layout/insights',
    methods: [
        'GET',
    ],
    summary: 'Get insights layout for an entity',
    description: 'Returns the insights layout for the given entity as an ordered list of insight panel entries.

**How to use:**
- `entityName` — the entity whose insights layout is requested.
...
- `layoutProfileId` — optional layout profile ID.',
    ...
    parameters: [
        ...
        [
            ...
            'in'          => 'query',
            'required'    => false,
            'description' => 'Custom layout name from `additionalLayouts`. Overrides the default `insights` view type.',
            'schema'      => [
                'type' => 'string',
            ],
        ],
    ...
    responses: [
        200 => [
            'description' => 'Insights layout for the requested entity',
            'content'     => [
                'application/json' => [
                    'schema' => [
                        'type'       => 'object',
                        ...
                        'properties' => [
                            'layout'            => [
                                'type'        => 'array',
                                'description' => 'Ordered list of insight panel entries.',
                                'items'       => [
...
class LayoutInsightsHandler extends AbstractHandler
{
    public function process(ServerRequestInterface $request, RequestHandlerInterface $handler): ResponseInterface
    {
        $qp       = $request->getQueryParams();
        $viewType = $qp['layoutName'] ?? 'insights';
        ...
        if (empty($data)) {
            throw new NotFound("Insights layout for '{$qp['entityName']}' is not found.");
        }
