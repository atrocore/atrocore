...
class Cluster extends Base
{
    ...
    public function getEntityClusterData(string $entityName, string $entityId): ?array
    {
        ...
        $this->prepareEntityForOutput($cluster);

        $allItems = $this->getEntityManager()
            ->getRepository('ClusterItem')
            ->where(['clusterId' => $cluster->get('id')])
            ->find();

        $goldenRecordId = $cluster->get('goldenRecordId');
        $clusterItemService = $this->getRecordService('ClusterItem');

        // Group item IDs per entity type for batch name resolution
        $idsByType = [];
        foreach ($allItems as $item) {
            $idsByType[$item->get('entityName')][] = $item->get('entityId');
        }

        $namesByType = [];
        foreach ($idsByType as $type => $ids) {
            $nameField = $this->getMetadata()->get(['scopes', $type, 'nameField']) ?? 'name';
            $select = array_unique(['id', $nameField]);
            $entities = $this->getEntityManager()->getRepository($type)->select($select)->where(['id' => $ids])->find();
            foreach ($entities as $e) {
                $namesByType[$type][$e->get('id')] = $e->get($nameField) ?? $e->get('id');
            }
        }
        ...
        foreach ($allItems as $item) {
            $itemEntityName = $item->get('entityName');
            ...
            if (!empty($this->getMetadata()->get(['scopes', $itemEntityName, 'primaryEntityId']))) {
                $stagingRecords[] = $row;
            } else {
                $masterRecords[] = $row;
            }
