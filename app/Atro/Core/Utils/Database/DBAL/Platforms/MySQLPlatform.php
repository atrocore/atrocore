...
SQL
    ...
    public function getAlterTableSQL(TableDiff $diff)
    {
        ...
        foreach ($diff->addedColumns as $column) {
            if ($column->getAutoincrement()) {
                $sql[] = 'ALTER TABLE ' . $diff->getName($this)->getQuotedName($this) . ' CHANGE ' . $column->getQuotedName($this) .' ' . $this->getColumnDeclarationSQL($column->getQuotedName($this), $column->toArray());
            }
        }
        // atro end.
    ...
    private function getPreAlterTableAlterIndexForeignKeySQL(TableDiff $diff): array
    {
        ...
        // atro start:
        foreach ($diff->removedIndexes as $removedIndex) {
            foreach ($removedIndex->getColumns() as $columnName) {
                foreach ($diff->fromTable->getColumns() as $column) {
                    if ($column->getName() == $columnName && $column->getAutoincrement()) {
                        $column->setAutoincrement(false);
