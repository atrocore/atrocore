...
class RDB extends \Espo\ORM\Repositories\RDB implements Injectable
{
    ...
    protected function validateInt(Entity $entity, string $fieldName, array $fieldData): void
    {
        ...
        if (is_int($value)) {
            ...
            if ($value < $min || $value > $max) {
                ...
                throw new BadRequest(
                    sprintf($this->getLanguage()->translate('integerValueValidationFailed', 'exceptions'), $label, $min, $max)
                );
            }
        }
    ...
    public function save(Entity $entity, array $options = [])
    {
        ...
        if (($entity->hasAttribute('modifiedAt')) && $entity->shouldApplyUpdatedFields() && ($entity->isNew() ? empty($entity->get('modifiedAt')) : !$entity->isAttributeChanged('modifiedAt'))) {
            $entity->set('modifiedAt', $nowString);
        }

        if ($entity->hasAttribute('modifiedById') && $entity->shouldApplyUpdatedFields() && $user) {
