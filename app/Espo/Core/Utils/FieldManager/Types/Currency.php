...
class Currency extends Type
{
    ...
    public function prepareSqlUniqueData(Entity $entity, string $field): array
    {
        ...
        return [
            'select' => "$table.$dbField AS `$field`, $table.{$dbField}_currency AS `{$field}Currency`",
            'where' => "($table.$dbField = '" . $entity->get($field) . "' AND $table.{$dbField}_currency = '" . $entity->get($field . 'Currency') . "')"
        ];
    }
    ...
    {
        return $entity->get($field) == $data[$field] && $entity->get($field . 'Currency') == $data[$field . 'Currency'];
    }
}