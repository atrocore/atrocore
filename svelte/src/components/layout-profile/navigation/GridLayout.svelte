<script lang="ts">
    ...
    function setupPanels() {
        lastPanelNumber = -1;
        lastRowNumber = -1
        let dashboardLayout = params.layout;
        let rowGroup = {};
        for (const dashboard of dashboardLayout) {
            let rowGroup = {};
            dashboard.layout.sort((a,b) => a.y - b.y);
            dashboard.layout.forEach(layout => {
                if(!rowGroup[layout.y]) {
                    rowGroup[layout.y] = [];
                }
                rowGroup[layout.y].push(layout);
            });
